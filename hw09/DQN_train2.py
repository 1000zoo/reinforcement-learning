...
if __name__ == "__main__":
    ...
    for e in range(EPISODES+1):
        ...
        while not done:
            ...
            # position 이 0.5 보다 크면 +10
            reward = -np.cos((state[0][0] + 0.5) * np.pi) + 1
            if state[0][0] >= 0.5 and not done:
                reward += 10
            ...
            if done:
                ...
                # score = -200 으로 done 이 된 경우, 해당 에피소드는
                # 목적지에 도달하지 못했다고 할 수 있다. (매 스텝마다 -1, 총 200스텝)
                # 즉, score > -200 이면 해당 에피소드는 성공했다고 볼 수 있다.
                if score > -200:
                    success += 1
                else:
                    success = 0
        ...
        if e % 100 == 0:
            agent.model.save_weights("./mountain_dqn.h5")
            pylab.plot(episodes, scores, 'b')
            pylab.savefig("./mountain_dqn.png")
